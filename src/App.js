...
function App() {
    ...
    const tables = 8;
    const zeros = Array.apply(null, Array(tables+1)).map(function(){return []})
    ...
    for (let i=1; i<=tables; i++) {
        tableArray.push(<GuestTable key={`table-${i}`} tableNumber={i} seatingChart={seatingChart} />)
    }
    ...
    const onDragEnd = result => {
        ...
        const newTable = (destination.droppableId === "guestBank")
        ? 0
        ...
        if (destination.droppableId !== source.droppableId) {
            ...
            updateSeatingChart(draggableId, newTable, source.index, destination.index)
        }
    }
